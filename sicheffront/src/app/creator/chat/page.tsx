...
export default function ChatInterface() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        ...
        { id: 2, text: "Hola que tal, un placer estar aqui!", sender: 'other', time: '10:05' },
    ]);
    ...
    const [users, setUsers] = useState<ChatUser[]>([
        {
            ...
            avatar: "https://images.unsplash.com/photo-1599566150163-29194dcaad36?w=600",
            unreadCount: 2,
            active: false
        },
        {
                    ...
                    {users.map((user) => (
                        <div
                            ...
                            className={`flex flex-row py-4 px-2 items-center border-b-2 border-gray-800 cursor-pointer transition relative
                                ${user.active ? 'bg-orange-500/20' : 'hover:bg-orange-500/10'}`}
                        >
                            <div className="w-1/4">
                                <img
                                    src={user.avatar}
                ...
                <div className="w-full px-5 flex flex-col justify-between h-[calc(100vh-100px)]">
                    <div className="flex flex-col mt-5 overflow-y-auto no-scrollbar">
                        {messages.map((msg) => (
                            ...
                            >
                                {msg.sender === 'other' && (
                                    <img src="https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=600" className="object-cover h-8 w-8 rounded-full mr-2" alt="Other" />
                                )}

                                <div className={`py-3 px-4 text-white max-w-[70%] break-words ${msg.sender === 'me'
                                    ? 'bg-[#F57C00] rounded-bl-3xl rounded-tl-3xl rounded-tr-xl mr-2'
                                    : 'bg-gray-600 rounded-br-3xl rounded-tr-3xl rounded-tl-xl ml-2'
                                    }`}>
                                    {msg.text}
                                    <div className="text-xs text-white/70 text-right mt-1">{msg.time}</div>
                                </div>

                                {msg.sender === 'me' && (
                                    <img src="https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=600" className="object-cover h-8 w-8 rounded-full" alt="Me" />
                                )}
                            </div>
                        ))}
                    </div>
                        ...
                        <form onSubmit={handleSendMessage} className="relative flex items-center">
                            ...
                            <button
                                ...
                                className={`absolute right-2 p-3 bg-[#F57C00] rounded-full text-white shadow-md transition-all duration-200 flex items-center justify-center
                                    ${inputValue.trim() ? 'hover:bg-orange-700 hover:scale-105' : 'opacity-50 cursor-not-allowed'}
                                `}
                            >
