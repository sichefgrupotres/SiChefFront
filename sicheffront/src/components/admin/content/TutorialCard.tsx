...
function TutorialCard({ tutorial }: TutorialCardProps) {
  return (
    <div className="bg-[#2a221b] p-4 rounded-xl border border-white/10 hover:border-orange-500 transition cursor-pointer">
      <h3 className="text-white font-semibold text-lg">{tutorial.title}</h3>
      <p className="text-white/60 text-sm">Autor: {tutorial.author}</p>
      <p className="text-white/60 text-sm">Categoría: {tutorial.category}</p>
    </div>
  );
}
export default function AdminContentPage() {
  const { data: session } = useSession();
  const { recipes, fetchRecipes, loading: loadingRecipes, error } = useRecipe();
